### Hey there

I'm Rayan, a full-stack developer based in France. I'm passionate about LLMs, currently experimenting with LangGraph and building small agent workflows on the side.

At Free Iliad, I work on REST APIs with FastAPI and Laravel, and set up the Docker infrastructure and GitLab CI/CD pipeline. 
Frontend side, the projects use Vue.js and PostgreSQL for storage.

Starting an MSc in Artificial Intelligence at Epitech this fall, and open to a work-study program (alternance) from October 2026.

---

**Tools & Tech**

- Python, FastAPI, Laravel
- Vue.js, PostgreSQL
- Docker, GitLab CI/CD

---

**A Bit About Me**

- Into video games and anime
- Play volleyball
- Always curious about new tech

---

**Contact**

[LinkedIn](https://linkedin.com/in/rayan-khan-704791389) · rayan.pro91@gmail.com
